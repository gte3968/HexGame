...
class Player{
  public:
    Player(int i): id(i){assert (id==1 || id==2); setSymbol();}
    void setSymbol(){if(id==1) symbol = 'X'; else symbol = 'O';}
    char getSymbol(){return symbol;}
    int getPlayerID(){return id;}
    pair<int, int> Move(HexGraph& g, Board& hBoard);  //make a move, returns <i,j> the position of the hex just placed
...
pair<int, int> Player::Move(HexGraph& g, Board& hBoard)
{
  int i, j;
  if(id==1){
...
bool Player::IfWon(HexGraph& g)
{
  for(auto ptr1=firstEdgeNodeList.begin(); ptr1<firstEdgeNodeList.end(); ++ptr1){  //any path between any node on the first edge and second edge
    for(auto ptr2=secondEdgeNodeList.begin(); ptr2<secondEdgeNodeList.end(); ++ptr2){
      int edge1Node = *ptr1;
      int edge2Node = *ptr2;
      ShortestPath sp1(g,edge1Node, edge2Node);
...
int main()
{
  ...
  while(true){
    pair<int, int> ij = p1.Move(g1, hexBoard);
    int iP1 = ij.first, jP1 = ij.second;
    ij = p2.Move(g1, hexBoard);
